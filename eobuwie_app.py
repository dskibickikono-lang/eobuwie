        ...
        if submit_upload and uploaded_file:
            with st.status("Processing Shift Report...", expanded=True) as status:
                try:
                    ...
                    if not id_col:
                        status.update(label="Could not find a worker ID column (e.g., 'login', 'Worker_ID').", state="error", expanded=False)
                    else:
                        ...
                        if not date_cols:
                            status.update(label="Could not identify any date columns in the file header.", state="error", expanded=False)
                        else:
                    ...
                    status.update(label=f"Error parsing file: {e}", state="error", expanded=False)
