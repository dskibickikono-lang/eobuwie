    ...
    with st.form("manual_entry_form", clear_on_submit=True):
        ...
        if submit_manual and w_id:
            if is_valid_worker_id(w_id):
                ...
                st.toast("Invalid Worker ID format. Use alphanumeric characters (2-15 chars).", icon="❌")
                ...
                try:
                    ...
                    if not id_col:
                        ...
                        if not date_cols:
                            ...
                            if dropped_count > 0:
                                status.update(label=f"Report successfully parsed! ({dropped_count} rows dropped due to invalid IDs)", state="complete", expanded=False)
                            else:
